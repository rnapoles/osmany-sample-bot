...
use App\TelegramBot\Types\ArrayOfChatMemberEntity;
use App\TelegramBot\Types\ArrayOfMessages;
use App\TelegramBot\Types\ArrayOfUpdates;
use App\TelegramBot\Types\Chat;
use App\TelegramBot\Types\ChatMember;
use App\TelegramBot\Types\File;
use App\TelegramBot\Types\Inline\QueryResult\AbstractInlineQueryResult;
use App\TelegramBot\Types\InputMedia\ArrayOfInputMedia;
use App\TelegramBot\Types\InputMedia\InputMedia;
use App\TelegramBot\Types\Message;
use App\TelegramBot\Types\Poll;
use App\TelegramBot\Types\Update;
use App\TelegramBot\Types\User;
use App\TelegramBot\Types\UserProfilePhotos;
use App\TelegramBot\Types\WebhookInfo;
...
class BotApi
{
   ...
   */
  public const CONTINUE_CODE = 100;
   ...
   * @throws \App\TelegramBot\InvalidJsonException
   */
  ...
  protected function executeCurl(array $options)
  {
    ...
    if (false === $result) {
      ...
      throw new HttpException(curl_error($this->curl), $errno);
    }
  ...
  public static function curlValidate($curl, $response = null)
  {
    ...
    if (($httpCode = curl_getinfo($curl, CURLINFO_HTTP_CODE))
            && !in_array($httpCode, [self::CONTINUE_CODE, self::DEFAULT_STATUS_CODE, self::NOT_MODIFIED_STATUS_CODE])
        ) {
  ...
  /**
   * Use this method to send text messages. On success, the sent \App\TelegramBot\Types\Message is returned.
   *
   ...
   *
   * @return \App\TelegramBot\Types\Message
   *
   * @throws \App\TelegramBot\InvalidArgumentException
   * @throws \App\TelegramBot\Exception
   */
   ...
   * @param int $limit
   *
   * @return \App\TelegramBot\Types\UserProfilePhotos
   *
   * @throws \App\TelegramBot\Exception
   */
   ...
   *
   * @return \App\TelegramBot\Types\WebhookInfo
   *
   * @throws \App\TelegramBot\Exception
   * @throws \App\TelegramBot\InvalidArgumentException
   */
   ...
   *
   * @return \App\TelegramBot\Types\User
   *
   * @throws \App\TelegramBot\Exception
   * @throws \App\TelegramBot\InvalidArgumentException
   */
   ...
   * @param $fileId
   *
   * @return \App\TelegramBot\Types\File
   *
   * @throws \App\TelegramBot\InvalidArgumentException
   * @throws \App\TelegramBot\Exception
   */
   ...
   *                            less than 30 seconds from the current time they
   ...
   * @param int      $userId               Unique identifier of the target user
   * @param int|null $untilDate            Date when restrictions will be lifted for the user, unix time.
   ...
   *                                 indirectly (promoted by administrators that were appointed by him)indirectly (promoted by administrators that were appointed by him)
   *
   * @return bool
   */
   ...
   * On success, the sent \App\TelegramBot\Types\Message is returned.
   *
   * @param int|string        $chatId
  ...
  public function sendMediaGroup(
    ...
    return ArrayOfMessages::fromResponse(
      $this->call(
              'sendMediaGroup',
              [
                'chat_id' => $chatId,
   ...
   * @param null   $type                  Poll type, “quiz” or “regular”,
   *                                      defaults to
   * @param bool   $allowsMultipleAnswers True, if the poll allows multiple answers,
   *                                      ignored for polls in quiz mode, defaults
   *                                        to False
   ...
   *                                      reply from the user.reply from the user.
   *
   * @return \App\TelegramBot\Types\Message
   *
   * @throws Exception
    ...
    return Message::fromResponse(
      $this->call(
              ...
              [
                ...
                'disable_notification' => (bool) $disableNotification,
                'reply_to_message_id' => (int) $replyToMessageId,
                'reply_markup' => null === $replyMarkup ? $replyMarkup : $replyMarkup->toJson(),
              ]
            )
    );
  }
   ...
   * On success, the stopped \App\TelegramBot\Types\Poll with the final results is returned.
   *
   * @param int|string $chatId
   * @param int        $messageId
   * @param  Types\ReplyKeyboardMarkup|Types\ReplyKeyboardHide|Types\ForceReply|
